...
var _ChainValidation = _interopRequireDefault(require("./ChainValidation"));

var _bigi = _interopRequireDefault(require("bigi"));

var _EmitterInstance = _interopRequireDefault(require("./EmitterInstance"));

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { "default": obj }; }

var object_type = _ChainTypes["default"].object_type,
    impl_object_type = _ChainTypes["default"].impl_object_type;
var emitter = (0, _EmitterInstance["default"])();
...
var default_auto_subscribe = true;
...
var ChainStore = /*#__PURE__*/function () {
  function ChainStore() {
  ...
  var _proto = ChainStore.prototype;
  ...
  _proto.clearCache = function clearCache() {
    ...
    */
    ...
    this.account_ids_by_key = _immutable["default"].Map();
    this.account_ids_by_account = _immutable["default"].Map();
  ...
  _proto.resetCache = function resetCache(subscribe_to_new) {
    this.subscribed = false;
    this.subError = null;
    this.clearCache();
  ...
  _proto.setDispatchFrequency = function setDispatchFrequency(freq) {
    this.dispatchFrequency = freq;
  };

  _proto.init = function init(subscribe_to_new) {
    var _this = this;

    if (subscribe_to_new === void 0) {
      subscribe_to_new = true;
    }
  ...
  _proto._subTo = function _subTo(type, id) {
    var key = "subbed_" + type;
    if (!this[key].has(id)) this[key].add(id);
  };

  _proto.unSubFrom = function unSubFrom(type, id) {
    var key = "subbed_" + type;
    this[key]["delete"](id);
    this.objects_by_id["delete"](id);
  };

  _proto._isSubbedTo = function _isSubbedTo(type, id) {
    var key = "subbed_" + type;
    return this[key].has(id);
  };

  _proto.onUpdate = function onUpdate(updated_objects /// map from account id to objects
  ) {
    var _this2 = this;
    ...
    for (var a = 0; a < updated_objects.length; ++a) {
      var _loop = function _loop(i) {
        ...
        if (_ChainValidation["default"].is_object_id(obj)) {
          ...
          switch (objectType) {
            ...
            case "proposal":
              _this2.subbed_accounts.forEach(function (acc) {
                ...
                if (current) {
                  var proposals = current.get("proposals", _immutable["default"].Set());
  ...
  _proto.unsubscribe = function unsubscribe(callback) {
  ...
  _proto.clearObjectCache = function clearObjectCache(id) {
    this.objects_by_id["delete"](id);
  }
  /**
  ...
  _proto.getObject = function getObject(id, force, autosubscribe, no_full_account) {
    if (force === void 0) {
      force = false;
    }

    if (autosubscribe === void 0) {
      autosubscribe = null;
    }

    if (no_full_account === void 0) {
    ...
    if (!_ChainValidation["default"].is_object_id(id)) throw Error("argument is not an object id: " + JSON.stringify(id));
  ...
  _proto.getAsset = function getAsset(id_or_symbol) {
    var _this4 = this;

    if (!id_or_symbol) return null;

    if (_ChainValidation["default"].is_object_id(id_or_symbol)) {
    ...
    if (_ChainValidation["default"].is_object_id(asset_id)) {
  ...
  _proto.getAccountRefsOfKey = function getAccountRefsOfKey(key) {
    var _this5 = this;
    ...
    if (this.get_account_refs_of_keys_calls.has(key)) return this.account_ids_by_key.get(key);else {
      ...
      _bitsharesjsWs.Apis.instance().db_api().exec("get_key_references", [[key]]).then(function (vec_account_id) {
        var refs = _immutable["default"].Set();
  ...
  _proto.getAccountRefsOfAccount = function getAccountRefsOfAccount(account_id) {
    var _this6 = this;
  ...
  _proto.getBalanceObjects = function getBalanceObjects(address) {
    var _this7 = this;
    ...
    if (current === undefined) {
       ...
       */
      this.balance_objects_by_address.set(address, _immutable["default"].Set());
      ...
      _bitsharesjsWs.Apis.instance().db_api().exec("get_balance_objects", [[address]]).then(function (balance_objects) {
        ...
        _this7.balance_objects_by_address.set(address, _immutable["default"].Set(set));
  ...
  _proto.fetchObject = function fetchObject(id, force, autosubscribe, no_full_account) {
    var _this8 = this;
    ...
    if (!_ChainValidation["default"].is_object_id(id)) throw Error("argument is not an object id: " + id);
  ...
  _proto.getAccount = function getAccount(name_or_id, autosubscribe) {
    if (autosubscribe === void 0) {
      autosubscribe = null;
    }
    ...
    if (_ChainValidation["default"].is_object_id(name_or_id)) {
      ...
      return account;
    } else if (_ChainValidation["default"].is_account_name(name_or_id, true)) {
  ...
  _proto.getAccountName = function getAccountName(id) {
  ...
  _proto.getWitnessById = function getWitnessById(account_id) {
  ...
  _proto.getCommitteeMemberById = function getCommitteeMemberById(account_id) {
  ...
  _proto.fetchAllWorkers = function fetchAllWorkers() {
    var _this9 = this;
  ...
  _proto.fetchWitnessByAccount = function fetchWitnessByAccount(account_id) {
    var _this10 = this;
  ...
  _proto.fetchCommitteeMemberByAccount = function fetchCommitteeMemberByAccount(account_id) {
    var _this11 = this;
  ...
  _proto.requestAllDataForAccount = function requestAllDataForAccount(account_id, object_type) {
    var current = this.objects_by_id.get(account_id);
    /***
     * TODO
    ...
    if (current.toJS().more_data_available.balances && object_type == "balance") {
      _bitsharesjsWs.Apis.instance().db_api().exec("get_account_balances", [account_id, []]).then(function (balances) {
  ...
  _proto.fetchFullAccount = function fetchFullAccount(name_or_id, autosubscribe) {
    var _this12 = this;
    ...
    var is_object_id = _ChainValidation["default"].is_object_id(name_or_id);

    var is_account_name = !is_object_id && _ChainValidation["default"].is_account_name(name_or_id, true);

    if (is_object_id && !subChanged) {
      var current = this.objects_by_id.get(name_or_id);
      fetch_account = current === undefined;
      if (!fetch_account && current && current.get && current.get("name") && current.has("balances")) return current;
    } else if (!subChanged) {
      if (!is_account_name) throw Error("argument is not an account name: " + name_or_id);
      var account_id = this.accounts_by_name.get(name_or_id);
      if (_ChainValidation["default"].is_object_id(account_id)) return this.getAccount(account_id, autosubscribe);
    ...
    if (subChanged || !this.fetching_get_full_accounts.has(name_or_id) || Date.now() - this.fetching_get_full_accounts.get(name_or_id) > 5000) {
      ...
      _bitsharesjsWs.Apis.instance().db_api().exec("get_full_accounts", [[name_or_id], autosubscribe]).then(function (results) {
        ...
        var account = full_account.account,
            ...
            more_data_available = full_account.more_data_available,
            ...
            htlcs_from = full_account.htlcs_from,
            htlcs_to = full_account.htlcs_to; // ensure backwards compatibility if node is not up-to-date

        if (!htlcs_from) {
          // available with 3.1.X
          htlcs_from = [];
        }
        ...
        account.assets = new _immutable["default"].List(assets || []);
        ...
        account.htlcs_to = new _immutable["default"].Set();
        account.htlcs_from = new _immutable["default"].Set();
        ...
        account.htlcs_to = account.htlcs_to.withMutations(function (set) {
          htlcs_to.forEach(function (htlc) {
            _this12._updateObject(htlc);

            set.add(htlc.id);
            if (autosubscribe) sub_to_objects.push(htlc.id);
          });
        });
        account.htlcs_from = account.htlcs_from.withMutations(function (set) {
          htlcs_from.forEach(function (htlc) {
            _this12._updateObject(htlc);
  ...
  _proto.getAccountMemberStatus = function getAccountMemberStatus(account) {
  ...
  _proto.getAccountBalance = function getAccountBalance(account, asset_type) {
  ...
  _proto.fetchRecentHistory = function fetchRecentHistory(account, limit) {
    var _this13 = this;
    ...
    if (!_ChainValidation["default"].is_object_id(account_id) && account.toJS) account_id = account.get("id");
    if (!_ChainValidation["default"].is_object_id(account_id)) return;
    ...
    pending_request.promise = new Promise(function (resolve, reject) {
      _bitsharesjsWs.Apis.instance().history_api().exec("get_account_history", [account_id, most_recent, limit, start]).then(function (operations) {
        ...
        if (!current_history) current_history = _immutable["default"].List();

        var updated_history = _immutable["default"].fromJS(operations);
  ...
  _proto._updateObject = function _updateObject(object, notify_subscribers, emit) {
    if (notify_subscribers === void 0) {
    ...
    */
    ...
    if (object.id == "2.1.0") {
      object.participation = 100 * ((0, _bigi["default"])(object.recent_slots_filled).bitCount() / 128.0);
    ...
    if (!current) {
      ...
      current = _immutable["default"].Map();
    }

    var prior = current;
    /* New object */

    if (current === undefined || current === true) this.objects_by_id.set(object.id, current = _immutable["default"].fromJS(object));else {
      /* Existing object */
      switch (objectType) {
        /*
        ...
        */
        ...
        case "asset_bitasset_data":
          this.objects_by_id.set(object.id, current = current.mergeDeep(_immutable["default"].fromJS(object)));
          break;

        /* Don't use merge deep to improve performance */

        default:
          this.objects_by_id.set(object.id, current = _immutable["default"].fromJS(object));
      }
    }
    ...
    switch (objectType) {
      case "account_balance":
        ...
        if (owner === undefined || owner === null || owner === true) {
          ...
          if (!balances) owner = owner.set("balances", _immutable["default"].Map());
      ...
      case "account":
        current = current.set("active", _immutable["default"].fromJS(object.active));
        current = current.set("owner", _immutable["default"].fromJS(object.owner));
        current = current.set("options", _immutable["default"].fromJS(object.options));
        current = current.set("whitelisting_accounts", _immutable["default"].fromJS(object.whitelisting_accounts));
        current = current.set("blacklisting_accounts", _immutable["default"].fromJS(object.blacklisting_accounts));
        current = current.set("whitelisted_accounts", _immutable["default"].fromJS(object.whitelisted_accounts));
        current = current.set("blacklisted_accounts", _immutable["default"].fromJS(object.blacklisted_accounts));
      ...
      case "asset":
        ...
        if (!bitasset && "bitasset_data_id" in object) {
          ...
          if (!bad) bad = _immutable["default"].Map();
      ...
      case "call_order":
        ...
        if (call_account && call_account !== true) {
          if (!call_account.has("call_orders")) call_account = call_account.set("call_orders", new _immutable["default"].Set());
      ...
      case "limit_order":
        ...
        if (limit_account && limit_account !== true) {
          if (!limit_account.has("orders")) limit_account = limit_account.set("orders", new _immutable["default"].Set());
  ...
  _proto.getObjectsByVoteIds = function getObjectsByVoteIds(vote_ids) {
    var _this14 = this;
  ...
  _proto.getObjectByVoteID = function getObjectByVoteID(vote_id) {
  ...
  _proto.getEstimatedChainTimeOffset = function getEstimatedChainTimeOffset() {
    ...
    var median_offset = _immutable["default"].List(this.chain_time_offset).sort().get(Math.floor((this.chain_time_offset.length - 1) / 2)); // console.log("median_offset", median_offset)


    return median_offset;
  };

  _proto.addProposalData = function addProposalData(approvals, objectId) {
    var _this15 = this;
    ...
    approvals.forEach(function (id) {
      ...
      if (impactedAccount && impactedAccount !== true) {
        didImpact = true;
        var proposals = impactedAccount.get("proposals", _immutable["default"].Set());
...
function FetchChainObjects(method, object_ids, timeout, subMap) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    function onUpdate(not_subscribed_yet) {
      if (not_subscribed_yet === void 0) {
        not_subscribed_yet = false;
      }
...
function FetchChain(methodName, objectIds, timeout, subMap) {
  if (timeout === void 0) {
    timeout = 3000;
  }
  ...
  return chain_store.FetchChainObjects(method, _immutable["default"].List(objectIds), timeout, subMap).then(function (res) {
