...
var ChainStore = /*#__PURE__*/function () {
  ...
  _proto.requestAllDataForAccount = function requestAllDataForAccount(account_id, object_type) {
    ...
    if (current.toJS().more_data_available.balances && object_type == "balance") {
      Apis.instance().db_api().exec("get_account_balances", [account_id, []]).then(function (balances) {
  ...
  _proto.getEstimatedChainTimeOffset = function getEstimatedChainTimeOffset() {
    ...
    var median_offset = Immutable.List(this.chain_time_offset).sort().get(Math.floor((this.chain_time_offset.length - 1) / 2)); // console.log("median_offset", median_offset)
