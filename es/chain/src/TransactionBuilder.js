...
var TransactionBuilder = /*#__PURE__*/function () {
  function TransactionBuilder(tx) {
    if (tx === void 0) {
      tx = null;
    }
  ...
  */
  ...
  _proto.process_transaction = function process_transaction(cwallet, signer_pubkeys, broadcast) {
    ...
    return this.set_required_fees().then(function () {
      ...
      if (signer_pubkeys) {
        ...
        for (var _iterator = _createForOfIteratorHelperLoose(pubkeys), _step; !(_step = _iterator()).done;) {
      ...
      return _this.get_potential_signatures().then(function (_ref) {
        ...
        return _this.get_required_signatures(my_pubkeys).then(function (required_pubkeys) {
          for (var _iterator2 = _createForOfIteratorHelperLoose(required_pubkeys), _step2; !(_step2 = _iterator2()).done;) {
  ...
  _proto.finalize = function finalize() {
    ...
    return new Promise(function (resolve, reject) {
      ...
      if (_this2.expiration !== 0 && _this2.ref_block_num !== 0 && _this2.ref_block_prefix !== 0) {
        _finalize();
        ...
        resolve(Apis.instance().db_api().exec("get_objects", [["2.1.0"]]).then(function (r) {
          ...
          if (_this2.ref_block_num === 0 && _this2.ref_block_prefix === 0) {
  ...
  */
  ...
  _proto.set_required_fees = function set_required_fees(asset_id, removeDuplicates) {
    var _this3 = this;
    ...
    return Promise.all(promises).then(function (results) {
      ...
      */
      ...
      var _loop = function _loop(_i) {
        ...
        if (isProposal(op)) {
          feeMap = updateFeeMap(feeMap, feeAssetId, _i, coreFees[_i][0]);
          op[1].proposed_ops.forEach(function (prop, y) {
            var propFeeAsset = prop.op[1].fee.asset_id;
            if (!proposalFeeMap[_i]) proposalFeeMap[_i] = {};
            if (!proposalFeeMap[_i][propFeeAsset]) proposalFeeMap[_i][propFeeAsset] = {
              total: 0,
              ops: [_i],
              propIdx: []
            };
            proposalFeeMap[_i] = updateFeeMap(proposalFeeMap[_i], propFeeAsset, y, coreFees[_i][1]);
          });
        } else {
          feeMap = updateFeeMap(feeMap, feeAssetId, _i, coreFees[_i]);
        }
      };
      ...
      for (var _i2 = 0; _i2 < _this3.operations.length; _i2++) {
        set_fee(_this3.operations[_i2][1], _i2);
      }
  ...
  _proto.get_required_signatures = function get_required_signatures(available_keys) {
    ...
    var tr_object = ops.signed_transaction.toObject(this); //DEBUG console.log('... tr_object',tr_object)
  ...
  _proto.add_signer = function add_signer(private_key, public_key) {
    ...
    for (var _iterator3 = _createForOfIteratorHelperLoose(this.signer_private_keys), _step3; !(_step3 = _iterator3()).done;) {
      var sp = _step3.value;
...
function _broadcast(was_broadcast_callback) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    var tr_object = ops.signed_transaction.toObject(_this5); // console.log('... broadcast_transaction_with_callback !!!')
    ...
    Apis.instance().network_api().exec("broadcast_transaction_with_callback", [function (res) {
      ...
      error.digest = hash.sha256(_this5.tr_buffer).toString("hex");
      error.transaction = _this5.tr_buffer.toString("hex");
      console.log(error);
